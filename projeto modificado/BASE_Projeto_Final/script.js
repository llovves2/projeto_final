...
// ─── PRODUTOS (apenas na página Index) ───────────────────────────────────────
...
function excluirProduto(id) {
  if (confirm("Tem certeza que deseja excluir este produto?")) {
    fetch(`http://localhost:3000/produto/${id}`, { method: "DELETE" })
      .then((res) => res.json())
      .then((dados) => { alert(dados.mensagem); carregarProdutos(); })
  ...
  formProduto.addEventListener("submit", (event) => {
    ...
    if (idEdicao) {
      fetch(`http://localhost:3000/produto/${idEdicao}`, {
        ...
        .then((res) => res.json())
        .then(() => { alert("Atualizado!"); carregarProdutos(); cancelarEdicao(); })
        .finally(() => { btn.disabled = false; btn.innerText = "Cadastrar Produto"; });
    } else {
      fetch("http://localhost:3000/produto", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        ...
        .catch((error) => {
          ...
          mensagemProduto.innerText = "Erro ao conectar com o servidor.";
        })
        .finally(() => { btn.disabled = false; btn.innerText = "Cadastrar Produto"; });
    }
  });
...
// ─── SELECTS (apenas na página de Pedidos) ───────────────────────────────────
...
async function carregarSelects() {
  try {
    ...
    produtos.forEach((p) => {
      listaPrecosProdutos[p.ID] = p.PRECO;
      selectP.innerHTML += `<option value="${p.ID}">${p.NOME}</option>`;
    });
  } catch (error) {
...
function atualizarTotal() {
  ...
  if (idProd && listaPrecosProdutos[idProd]) {
    campoTotal.value = (listaPrecosProdutos[idProd] * qtd).toFixed(2);
  }
}

// ─── PEDIDOS (apenas na página pedido.html) ──────────────────────────────────
...
function preencherFormPedido(id, idCliente, idProduto, valor, status) {
  ...
  document.getElementById("btnCancelarPedido").style.display = "inline";
}
...
function excluirPedido(id) {
  if (confirm("Tem certeza que deseja excluir este pedido?")) {
    fetch(`http://localhost:3000/pedido/${id}`, { method: "DELETE" })
      .then((res) => res.json())
      .then((dados) => { alert(dados.mensagem); carregarPedidos(); })
      .catch((err) => console.error("Erro ao excluir:", err));
  }
}
...
// ─── CLIENTES (apenas na página cliente.html) ────────────────────────────────
...
function excluirCliente(id) {
  if (confirm("Tem certeza que deseja excluir este cliente?")) {
    fetch(`http://localhost:3000/cliente/${id}`, { method: "DELETE" })
      .then((res) => res.json())
      .then((dados) => { alert(dados.mensagem); carregarClientes(); })
      .catch((err) => console.error("Erro ao excluir:", err));
  }
}
